...
class Biblioteca(commands.Cog):
    ...
    async def criar_origem(
        ...
        try:
            self.cursor.execute('INSERT INTO origens (nome, descricao, categoria, atributo_bonus) VALUES (?, ?, ?, ?)', (nome, descricao, categoria, atributo_bonus))
            self.conn.commit()
    ...
    async def ver_origens(
        ...
        if nome:
            self.cursor.execute('SELECT nome, descricao, categoria, atributo_bonus FROM origens WHERE nome = ?', (nome,))
            origem = self.cursor.fetchone()
            ...
            self.cursor.execute('SELECT nome, categoria, atributo_bonus FROM origens')
    ...
    async def editar_origem(
        ...
        self.cursor.execute('UPDATE origens SET nome = ?, descricao = ?, categoria = ?, atributo_bonus = ? WHERE nome = ?', 
    ...
    async def deletar_origem(self, ctx, nome: str):
        self.cursor.execute('DELETE FROM origens WHERE nome = ?', (nome,))
        self.conn.commit()
    ...
    async def criar_aptidao(self, ctx, nome: str, custo: int, descricao: str):
        try:
            self.cursor.execute('INSERT INTO biblioteca (nome, descricao, tipo, custo) VALUES (?, ?, ?, ?)',
    ...
    async def ver_aptidoes(
        ...
        if nome:
            self.cursor.execute("SELECT nome, descricao, custo FROM biblioteca WHERE nome = ? AND tipo = 'Aptidão'", (nome,))
            aptidao = self.cursor.fetchone()
            ...
            self.cursor.execute("SELECT nome, custo FROM biblioteca WHERE tipo = 'Aptidão'")
    ...
    async def editar_aptidao(
        ...
        self.cursor.execute("UPDATE biblioteca SET nome = ?, descricao = ?, custo = ? WHERE nome = ? AND tipo = 'Aptidão'", 
    ...
    async def deletar_aptidao(self, ctx, nome: str = discord.Option(str, description="Nome da aptidão")):
        self.cursor.execute("SELECT id FROM biblioteca WHERE nome = ? AND tipo = 'Aptidão'", (nome,))
        res = self.cursor.fetchone()
        
        if not res:
            return await ctx.respond("❌ Aptidão não encontrada.", ephemeral=True)

        self.cursor.execute('DELETE FROM posses WHERE conhecimento_id = ?', (res[0],))
        self.cursor.execute('DELETE FROM biblioteca WHERE id = ?', (res[0],))
        self.conn.commit()
